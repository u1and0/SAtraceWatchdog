...
def allplt_wtf(df,
    ...
    * ウォータフォールをイメージプロット(countourf plot)"""
    ...
    ax = df.T.plot(legend=False, linewidth=.2, ylim=(-119, -20),
                   ax=ax1)  # スペクトラムプロット
    ax = df.max().loc[df.marker].plot(style='rD',
                                      markeredgewidth=1,
                                      fillstyle='none',
                                      ax=ax1,
                                      markersize=3.5)  # マーカープロット
    plt.ylabel(yzlabel)
    ...
    ax = plt.contourf(x, y, z, interval, alpha=.75, cmap=cmap,
                      extend=extend)  # ウォーターフォールプロット
